...
HAVING COUNT(v2.id) <> 1").Any())
        ...
        protected override void ReplaceColumn<T>(string tableName, string currentName, string newName)
        {
        ...
        private void MigratePropertyData()
        {
            ...
            if (ColumnType(PreTables.PropertyData, "versionId") == "uniqueidentifier")
            {
                ...
                Database.Execute($"UPDATE pd SET pd.versionId2=cv.id FROM {PreTables.ContentVersion} cv INNER JOIN {PreTables.PropertyData} pd ON cv.versionId = pd.versionId");
...
SELECT nodeId FROM {PreTables.ContentVersion} cv INNER JOIN {Constants.DatabaseSchema.Tables.DocumentVersion} dv ON dv.id = cv.id WHERE dv.published=1)");
        ...
        private class PropertyDataVersionId2
        {
